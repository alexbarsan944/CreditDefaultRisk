...
def build_model_input():
    ...
    buro_bal = pd.concat(
        [buro_bal, pd.get_dummies(buro_bal.STATUS, prefix="buro_bal_status")], axis=1
    ).drop("STATUS", axis=1)

    print("Counting buros")
    buro_counts = (
        buro_bal[["SK_ID_BUREAU", "MONTHS_BALANCE"]].groupby("SK_ID_BUREAU").count()
    )
    ...
    buro_full = pd.concat(
        [buro, buro_credit_active_dum, buro_credit_currency_dum, buro_credit_type_dum],
        axis=1,
    )
    ...
    nb_bureau_per_curr = (
        buro_full[["SK_ID_CURR", "SK_ID_BUREAU"]].groupby("SK_ID_CURR").count()
    )
    buro_full["SK_ID_BUREAU"] = buro_full["SK_ID_CURR"].map(
        nb_bureau_per_curr["SK_ID_BUREAU"]
    )
    ...
    for f_ in prev_cat_features:
        prev_dum = pd.concat(
            [prev_dum, pd.get_dummies(prev[f_], prefix=f_).astype(np.uint8)], axis=1
        )
    ...
    cc_bal = pd.concat(
        [
            cc_bal,
            pd.get_dummies(cc_bal["NAME_CONTRACT_STATUS"], prefix="cc_bal_status_"),
        ],
        axis=1,
    )
    ...
    test = sanitize_feature_names(test)
...
def train_model(data_, test_, y_, folds_):
    ...
    df_oof_preds = pd.DataFrame(
        {"SK_ID_CURR": ids, "TARGET": y, "PREDICTION": oof_preds}
    )
...
if __name__ == "__main__":
    ...
    oof_preds, df_oof_preds, test_preds, importances, score = train_model(
        data, test, y, folds
    )
    ...
    sub_file = (
        "submission_5x-average-LGB-run-01-v1_"
        + score
        + "_"
